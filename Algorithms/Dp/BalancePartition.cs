...
namespace Dp
{
    public class BalancePartitionSample
    {
        ...
        private void Test(BalancePartition partition, int[] nums, int sum)
        {
            ...
            bool isSubset = partition.IsSubsetSum(nums, sum);
    ...
    public class BalancePartition
    {
        ...
        public bool IsSubsetSum(int[] nums, int sum)
        {
            ...
            if (nums[0] <= sum) matrix[0, nums[0]] = nums[0];
            for (int i = 1; i < nums.Length; i++)
            {
                for (int j = 1; j < sum + 1; j++)
                {
                    if (j >= nums[i])
                    {
                        matrix[i, j] = matrix[i - 1, j - nums[i]] + nums[i];
                    }
                }
            }
        ...
        public bool IsSubsetSum1(int[] nums)
        {
            int total = 0;
            foreach (int num in nums)
            {
